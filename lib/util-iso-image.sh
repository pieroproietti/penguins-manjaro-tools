...
configure_thus(){
    ...
    echo "[install]" >> "$conf"
    echo "LIVE_MEDIA_SOURCE = \"/run/pe_miso/bootmnt/${iso_name}/${target_arch}/rootfs.sfs\"" >> "$conf"
    echo "LIVE_MEDIA_DESKTOP = \"/run/pe_miso/bootmnt/${iso_name}/${target_arch}/desktopfs.sfs\"" >> "$conf"
